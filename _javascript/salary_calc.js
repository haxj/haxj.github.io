...
let nf_en = new Intl.NumberFormat('en-US');
let num2str = function (n) {
    if (isNaN(n))
        n = 0;
    return nf_en.format(n).replaceAll(',', ' ');
};
...
[...document.getElementsByClassName("input-number")].forEach(function(input) {
    input.addEventListener('keyup', function(evt) {
        ...
        if (evt.target.selectionStart === evt.target.selectionEnd) {
            ...
            let n = parseInt(this.value.replaceAll(/\D/g, ''), 10);
            input.value = num2str(n);
...
document.getElementById("set-insr-max").onclick = function() {
    let gross_monthly = parseInt(document.getElementById("gross-income").value.replaceAll(/\D/g, ''), 10);
    if (document.getElementById("income-period").value === "annual") {
        gross_monthly /= 12;
    }
    document.getElementById("insurance-base").value = num2str(Math.round(gross_monthly));
}
...
class TaxPolicy {
    constructor() {
        ...
        this.per_dependent_deduction = 4400.0;
...
class Calculator {
    ...
    calculate() {
        ...
        this.desc.push({
            ...
            details: [{ name: 'Thu nhập thực nhận', formula: `${num2str(Math.round(this.info.gross))} - ${num2str(Math.round(this.insurance.total()))} - ${num2str(Math.round(this.tax.total))}`, value: this.info.net }]
        });
    }
    ...
    calculate_insurance() {
        ...
        this.desc.push({
            ...
            details: [
                ...
                { name: 'Mức đóng BHXH hàng tháng',              formula: `${num2str(this.policy.insurance.bhxh_rate * 100)}% x ${num2str(Math.round(this.insurance.base_bhxh))}`,                 value: this.insurance.bhxh      },
                { name: 'Mức đóng BHYT hàng tháng',              formula: `${num2str(this.policy.insurance.bhyt_rate * 100)}% x ${num2str(Math.round(this.insurance.base_bhyt))}`,                 value: this.insurance.bhyt      },
                { name: 'Mức đóng BHTN hàng tháng',              formula: `${num2str(this.policy.insurance.bhtn_rate * 100)}% x ${num2str(Math.round(this.insurance.base_bhtn))}`,                 value: this.insurance.bhtn      },
                { name: 'Tổng đóng bảo hiểm hàng tháng',         formula: `${num2str(Math.round(this.insurance.bhxh))} + ${num2str(Math.round(this.insurance.bhyt))} + ${num2str(Math.round(this.insurance.bhtn))}`, value: total_monthly   },
            ]
        });
        ...
        if (this.options.annual) {
            ...
            this.desc[this.desc.length-1].details.push(
                { name: 'Tổng đóng bảo hiểm hàng năm',         formula: `${num2str(Math.round(total_monthly))} x 12`, value: this.insurance.total()   },
            );
        }
    }
    ...
    calculate_tax() {
        ...
        this.desc.push({
            title: 'Thuế ',
            details: [
                { name: 'Thu nhập chịu thuế',                    formula: `${num2str(Math.round(this.info.gross))} - ${num2str(Math.round(this.insurance.total()))}`,       value: this.tax.gross_no_insurance  },
                { name: 'Giảm trừ gia cảnh cho bản thân',        formula: '',                                                                                               value: this.tax.standard_deduction  },
                { name: 'Giảm trừ gia cảnh cho người phụ thuộc', formula: `${num2str(this.policy.tax.per_dependent_deduction)} x ${this.info.n_dependents}`,                value: this.tax.total_dep_deduction },
                { name: 'Tổng giảm trừ gia cảnh',                formula: `${num2str(this.tax.standard_deduction)} + ${this.tax.total_dep_deduction}`,                      value: this.tax.total_deduction     },
                { name: 'Thu nhập tính thuế',                    formula: `${num2str(Math.round(this.tax.gross_no_insurance))} - ${num2str(this.tax.total_deduction)}`,     value: this.tax.taxable             },
                { name: 'Trong đó',                              formula: '',                                                                                               value: null                         },
            ]
        });
        ...
        while (this.tax.taxable >= this.policy.tax.tax_rate[i + 1][0]) {
            ...
            this.desc[2].details.push({ name: `Thuế bậc ${i+1}`, formula: `${num2str(Math.round(this.tax.taxable_level[i]))} x ${this.policy.tax.tax_rate[i][1] * 100}%`, value: this.tax.tax_level[i] });
            formula_total_tax += `${num2str(Math.round(this.tax.tax_level[i]))} + `
...
function getInfoFromUI() {
    let gross_monthly = Number.parseInt(document.getElementById("gross-income").value.replaceAll(' ', ''));
    if (gross_monthly < 0) {
        return;
    }
    ...
    let insr_base = Number.parseInt(document.getElementById("insurance-base").value.replaceAll(' ', ''));
    if (insr_base !== 0)
    ...
    if (currency === "usd") {
        exchg_rate = Number.parseInt(document.getElementById("exchg-rate").value.replaceAll(' ', ''));
    ...
    if (insr_base > gross_monthly) {
        ...
        if (currency === "usd")
            document.getElementById("insurance-base").value = num2str(Math.round(insr_base / exchg_rate));
        else
            document.getElementById("insurance-base").value = num2str(Math.round(insr_base));
    }
...
function updateUI() {
    document.getElementById("res-gross").innerHTML = `${num2str(calc.info.gross)}`;
    document.getElementById("res-insurance").innerHTML = `${num2str(calc.insurance.total())}`;
    document.getElementById("res-tax").innerHTML = `${num2str(calc.tax.total)}`;
    document.getElementById("res-net").innerHTML = `${num2str(calc.info.net)}`;
    ...
    calc.desc.forEach(desc => {
        ...
        desc.details.forEach((item, i, details) => {
            ...
            if (!item.value) {
                ...
                cell.appendChild(document.createTextNode(`${num2str(Math.round(item.value))}`));
...
function draw_pie(container_id) {
    ...
    arc.append("title")
        .text(d => `${d.data.name}\n${num2str(Math.round(d.data.value))} VND`);
...
function draw_sankey(container_id) {
    ...
    const nodeSort = null;
    const linkSort = null;
    const nodeAlignment = sankeyJustify;
    let nodeTextSize = 16;
    ...
    svgNodes.append("title")
            ...
            .text(d => `${num2str(Math.round(d.value))} VND`);
    ...
    svgNodes
            .on("mouseenter", function(e, d) {
                ...
                parent_node.select("text").append("tspan")
                        ...
                        .attr("pointer-events", "none")
                        .attr("font-size", nodeTextSize)
                        .text(`${num2str(Math.round(d.value))} VND`)
